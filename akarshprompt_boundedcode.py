...
def make_cfg() -> Config:
    ...
    if fast:
        cfg.num_tasks = 5; cfg.classes_per_task = 5
    ...
    cfg.run_tag = f"akarsh_{mode}_{cfg.data_split}_{cfg.classifier}{rcp_tag}{cap_tag}{util_tag}{fast_tag}"
    return cfg
...
class Server:
    ...
    def _utility_evict_clean(self, vit, val_loaders, seen_tasks, sim):
        """Improved utility eviction: (1) per-task loss signal (not global accuracy,
        so classifier drift on other tasks doesn't corrupt it); (2) smooth loss, not
        accuracy; (3) full val set; (4) noise-aware tie-break -> drop randomly among
        near-equal slots, so on redundant data it degrades to random (never worse)."""
        ...
        for sid in self.slot_ids:
            ...
            inc = [pool_task_loss(vit, self, keep, val_loaders[t], self.cfg, seen_hi) - base[t]
                   for t in tasks]
            util[sid] = sum(inc)/len(inc) if inc else 0.0   # small loss-rise => least needed
        mn = min(util.values())
...
def pool_task_loss(vit, server, slot_ids, val_loader, cfg, seen_hi):
    ...
    Smooth (loss, not accuracy) and task-isolated -> a clean utility signal not
    corrupted by classifier drift on other tasks."""
    prompts = [server.prompt_of[s] for s in slot_ids]
...
def main():
    ...
    ref = PAPER.get(cfg.data_split, (None,None))

    print("="*78)
    print(f"  akarshprompt BOUNDED | Fed-CPrompt | CIFAR-100 | {cfg.num_tasks}x{cfg.classes_per_task}")
    ...
    NW = int(os.environ.get("AK_WORKERS", "2"))   # set AK_WORKERS=0 if /dev/shm crash
    ...
    test_loaders = {t: DataLoader(GlobalLabelDS(test_ds, test_idx[t], global_of), 256, shuffle=False,
                                  num_workers=NW, pin_memory=True) for t in range(cfg.num_tasks)}
    ...
    def _mk_loaders(splits):
        return [DataLoader(GlobalLabelDS(train_ds, ci, global_of), cfg.batch_size, shuffle=True,
                           num_workers=NW, pin_memory=True) if len(ci) > 0 else None for ci in splits]
