...
def build_medmnist(cfg: Config):
    ...
    def parts(split, tf):
        ...
        return out
    train = MedConcat(parts("train", train_tf), C)
    test  = MedConcat(parts("test",  test_tf),  C)
    g = np.random.RandomState(cfg.seed)
    def per_task(ds):
        return [np.where((ds.targets >= t*C) & (ds.targets < (t+1)*C))[0].tolist()
                for t in range(cfg.num_tasks)]
    # AK_MAX_PER_TASK is a CEILING on train+val images per task: any dataset larger
    # than it is trimmed; smaller ones keep all their data. 0 = no cap.
    # (AK_MAX_PER_TASK=20000 -> Path 36k->20k, OCT 88k->20k, rest untouched.)
    MAXPT   = int(os.environ.get("AK_MAX_PER_TASK", "0"))
    MAXTEST = int(os.environ.get("AK_MAX_TEST", "0"))
    train_idx, val_idx = [], []
    for idxs in per_task(train):
        a = np.array(idxs); g.shuffle(a)
        if MAXPT > 0: a = a[:MAXPT]
        cut = int((1 - cfg.val_frac) * len(a))
        train_idx.append(a[:cut].tolist()); val_idx.append(a[cut:].tolist())
    test_pt = per_task(test)
    if MAXTEST > 0:
        test_pt = [list(g.permutation(np.array(idx))[:MAXTEST]) for idx in test_pt]
    global_of = {i: i for i in range(cfg.num_tasks * C)}
    print("  MedMNIST cross-domain tasks (first %d classes each):" % C)
    for t, f in enumerate(flags):
        print(f"    task {t+1}: {f:14s} -> labels {t*C}-{(t+1)*C-1} | "
